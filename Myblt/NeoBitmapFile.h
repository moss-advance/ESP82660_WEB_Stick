...
// 可以设置亮度的
#include "MyNeoShaderNop.h"
    MyNeoShaderNop<typename T_COLOR_FEATURE::ColorObject> myShaderNop;
    void MyBlt(NeoBufferContext<T_COLOR_FEATURE> destBuffer,
    uint16_t indexPixel,
    int16_t xSrc,
    int16_t ySrc,
    int16_t wSrc,
    uint8_t brightness)
    {
        myShaderNop.set_Brightness(brightness);
        Render<MyNeoShaderNop<typename T_COLOR_FEATURE::ColorObject>>(destBuffer, myShaderNop, indexPixel, xSrc, ySrc, wSrc);
    };
    template <typename T_SHADER> void Render(NeoBufferContext<T_COLOR_FEATURE> destBuffer,
        T_SHADER& shader,
        int16_t xDest,
